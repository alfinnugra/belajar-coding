...
function countProfit(shoppers) {
    ...
    for(let i = 0; i < listBarang.length; i++) {
        ...
        for(let x = 0; x < shoppers.length; x++) {
            if(listBarang[i][0] == shoppers[x].product && shoppers[x].amount <= barang.leftOver) {
                barang.shoppers.push(shoppers[x].name) 
                barang.leftOver-=(shoppers[x].amount)
                barang.totalProfit = (listBarang[i][2] - barang.leftOver) * listBarang[i][1]
            }
        }
        hasil.push(barang)
    }
